...
fn main() {
    ...
    // Deliberately the product name, not `branding::APP_NAME`: TERM_PROGRAM is an identity shell
    // integrations match on, so a development build must not look like a different terminal.
    ...
    window.setTitle(&NSString::from_str(branding::APP_NAME));
