...
- The window is one flat surface: the card's fill, the terminal and the gutter between them are all the theme background, and the hairline is the only thing separating them. The card therefore carries **no drop shadow** — a shadow tints the gutter it falls into, which is the seam this layout exists to avoid.
